	   ...
	   <div class="row">
		<div class="col-lg-12">
         ...
         <div class="headline">
		  ...
		  <a href="<?=$this->siteUrl()?>/client/dashboard/add"><?=$this->lang('add_project')?></a>
		 </div>	
		...
		<?php elseif($data['has'] === true): ?> 
		  ...
		  <?php if($data['projects_has_completed'] === false): ?> 
		  
			  <div class="listing-info text-center">
				 <h3><?=$this->lang('you_have_no_completed_projects')?>.</h3>
		 ...
		 <?php elseif($data['projects_has_completed'] === true): ?> 
		
	   
			<?php foreach($data['projects_completed'] as $row) { ?>	
				 ...
				 <div class="project-box">
				  ...
				  <div class="project-details">
				   <div class="project-description">
					...
					<?php foreach($data['projects_payments'] as $key=>$name){
						if($row['projectid'] == $key){ ?>
							<h4> <?=$this->lang('amount_paid')?> - <?=e($this->currency)?><?=e($name)?></h4>
					<?php } }?> 
					<?php foreach($data['projects_user'] as $r2){
					 if($r2['userid'] == $row['freelancerid']){ ?>
					...
					<h4> <?=$this->lang('complete_date')?> - <?=strftime("%b %d, %Y", strtotime($row['complete_date'])); ?></h4>
				   </div>
				  </div><!--/ .job-box-details -->
				  <div class="project-bid">
				   <div class="project-bid-inner">
					<a href="<?=$this->siteUrl()?>/<?=$this->client_url()?>/addrating/<?=e($row["projectid"])?>" class="kafe-btn kafe-btn-yellow"> <i class="fa fa-star"></i> <?=$this->lang('rate')?> <?=$this->freelancer_name()?></a>
				   </div>
				  </div><!--/ .project-bid -->	
				 </div><!--/ .project-box -->	
