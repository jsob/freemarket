...
?>
     ...
     <section class="navbar-box text-center">
      <ul id="">
       <li><a href="<?=$this->siteUrl()?>/<?=$this->clients_url()?>/projects/<?=e($data['user']["userid"])?>/<?=e($data['user']["slug"])?>"><?=$this->lang('projects')?> 
	     <em class="hidden-xs hidden-sm">(<?=e($data['total_projects'])?>)</em></a></li>
       <li class="active"><a href="<?=$this->siteUrl()?>/<?=$this->clients_url()?>/ratings/<?=e($data['user']["userid"])?>/<?=e($data['user']["slug"])?>"><?=$this->lang('ratings')?> 
      ...
      <div class="container">
       ...
       <div class="row">
		...
		<?php elseif($data['has_ratings'] === true): ?> 
		   ...
		   <?php foreach($data['client_projects'] as $row) { ?>  

				 <div class="ratings-box">	 
				  <h4 class="ratings-title"><?=e($row['title'])?></h4>
				  
				<?php foreach($data['client_ratings'] as $key=>$name){
