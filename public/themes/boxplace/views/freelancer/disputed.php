      ...
      <div class="container gal-container">
	   <div class="row">
		...
		<?php elseif($data['f_has'] === true): ?> 
		  ...
		  <?php if($data['has_disputed_projects'] === false): ?> 
		  
			  <div class="listing-info text-center">
				 <h3><?=$this->lang('you_have_no_disputed_projects')?>.</h3>
			  </div><!-- /.prop-info -->
			  
		 <?php elseif($data['has_disputed_projects'] === true): ?> 
		
	   
			<?php foreach($data['disputed_projects'] as $row) { ?>	
				 ...
				 <div class="project-box">
				  ...
				  <div class="project-details">
				   <div class="project-description">
					...
					<?php foreach($data['f_projects_payments'] as $key=>$name){
					...
					<h4> <?=$this->lang('disputed_date')?> - <?=strftime("%b %d, %Y", strtotime($row['disputed_date'])); ?></h4>
				   </div>
				  </div><!--/ .job-box-details -->
				  <div class="project-bid">
				   <div class="project-bid-inner">
					<?php foreach($data['get_dispute_conversation'] as $key=>$name){
						if($row['projectid'] == $key && $row['userid'] == $name["clientid"]){ ?>
						<a href="<?=$this->siteUrl()?>/<?=$this->freelancer_url()?>/dispute/view/<?=e($row["freelancerid"])?>/<?=e($name["cid"])?>" class="kafe-btn kafe-btn-yellow"><?=$this->lang('send_message')?></a>
						<?php } }?>
				   </div>
				  </div><!--/ .project-bid -->	
				 </div><!--/ .project-box -->	
