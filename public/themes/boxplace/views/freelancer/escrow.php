			   ...
			   <div class="withdraw-description">
				<?php foreach($data['users'] as $r1){
				 if($r1['userid'] == $row['clientid']){ ?>
					<h4><?=$this->lang('you_gor_hired_by')?> - <a href="<?=$this->siteUrl()?>/<?=$this->clients_url()?>/projects/<?=e($r1["userid"])?>/<?=e($r1["slug"])?>" target="_blank"> "<?=e($r1["name"])?>"</a></h4>
				<?php } }?> 
			  ...
			  <div class="withdraw-bid">
			   <div class="withdraw-bid-inner">
				...
				<?php elseif(e($row["action"]) === "2"): ?> 
					 <h5><?=$this->lang('money_released_to_you')?>.</h5>
				<?php elseif(e($row["action"]) === "3"): ?> 
					 <h5><?=$this->lang('money_in_dispute')?>.</h5>
				<?php elseif(e($row["action"]) === "4"): ?> 
					 <h5><?=$this->lang('money_returned_to')?> <?=$this->client_name()?>.</h5>
				<?php endif; ?> 
